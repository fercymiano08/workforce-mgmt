...
| 5 | `scheduler-attendance`, `-intelligence`, `-scheduling`, `-timeoff`, `-payroll` | The **same images**, but instead of serving web requests they run `php artisan schedule:work` — the background jobs (the once-a-minute data sync in all five; the attendance one also runs the hourly "sick certificate overdue" check, a once-a-minute re-count of recent days' hours, and the nightly job that records finished no-show days as Absent (00:10 Manila time); the scheduling one also runs the hourly automatic shift assignment (off until an administrator turns it on); the payroll one also runs the hourly timesheet jobs: auto-submit at the Monday-noon deadline, and reminders). In the normal scripts these are the hidden PowerShell windows | none |
