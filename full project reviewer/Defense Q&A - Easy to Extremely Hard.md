...
Yes, which is a good sign the test was worth doing: a missing import that would have crashed the Audit Logs page, invisible BOM characters in 7 services' bootstrap files, read-only cache folders copied from Windows, and a PostgreSQL start-up timing issue. All were fixed and re-tested (the full PHPUnit suite passed — it now has 321 tests; a from-scratch `down -v` + `up -d` produces a working system in about 80 seconds).
