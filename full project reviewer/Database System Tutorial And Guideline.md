...
| `attendance_adjustments` | Corrections (Time & Attendance): a request when the record is wrong because something stopped it being recorded - `worked_past_shift`, `kiosk_clock_in` or `kiosk_clock_out`. Holds what the employee said (claimed time, reason, 1-5 proof photos with captions), what the system derived from the shift, the time the admin finally entered (`final_time`), the kiosk-record checks frozen at filing (`corroboration`, json), and who decided | `id`, `employee_id`, `date`, `type`, `claimed_time`, `final_time`, `proof`, `derived_hours`, `status`, `decided_by` |
