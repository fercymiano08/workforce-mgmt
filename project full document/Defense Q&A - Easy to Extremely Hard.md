...
The kiosk is a shared device at the entrance, not a personal account. Its endpoints are intentionally public but return only minimal fields (name, photo, department, today's schedule) — never salary, email, phone, or address — so there's nothing sensitive to protect even without a login.
...
Correct catch — right now, checking "who is this token?" costs a real network round-trip on every single authenticated request to every service, which is slower than it needs to be and adds load to `core`. The standard fix is a short-lived cache (a few seconds) of token → user results inside each service, so repeated requests from the same user don't all hit `core`. We'd name this as our top performance improvement if pushed on "how would you scale this."
